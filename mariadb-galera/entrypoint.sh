...
# https://github.com/panubo/docker-mariadb-galera/blob/master/galera-entrypoint.sh

[ "$DEBUG" == 'true' ] && set -x

set -eo pipefail

export WSREP_CLUSTER_ADDRESS=gcomm://$(ip addr | grep -P -o '\d+\.(?!255)\d+\.\d+\.\d+(?=/32)'):4567
export WSREP_NODE_ADDRESS=$(ip addr | grep -P -o '\d+\.\d+\.\d+\.\d+(?=/24)')
#export WSREP_CLUSTER_ADDRESS=$WSREP_NODE_ADDRESS
#export WSREP_NODE_ADDRESS=$WSREP_CLUSTER_ADDRESS
env
